...
export class SearchRelatedAnchorsHandler implements IQueryHandler<SearchRelatedAnchorsQuery, SearchRelatedAnchorsQueryResponse> {
  ...
  async execute(query: SearchRelatedAnchorsQuery): Promise<SearchRelatedAnchorsQueryResponse> {
    // Get embedding for the search text using embeddings service
    const queryEmbedding = await this.embeddingsService.generateEmbedding(query.searchText);

    // Search for related anchors in Qdrant
    const searchResults = await this.qdrantService.searchAnchors(queryEmbedding, 100);
